...
def main(obj_name, algo_name, env_name, task_name, constraint_threshold=0.05):
    ...
    if algo_name == 'ModRew':
        csv_header = [
            ...
            'observe_convergence','run_time','check_CE','check_CE_origame'
        ]
    else:
        csv_header = [
            'Environment', 'Task', 'SolutionConcept', 'Method', 'Epoch',
            'Iteration', 'Risk', 'ExpectedReward', 'BFViolated', 'RegretViolated',
            'observe_convergence','run_time','check_CE'
        ]
    ...
    policy_file_name = "./Result/" + obj_name + algo_name + env_name + task_name
    ...
    for epc in range(int(arglist.num_runs)):  # Each experiment
            ...
            if algo_name != 'ModRew':
                if dif_qts + dif_om <= stop_threshold:  # or dif_om <= om_stop_thres:
                    ...
                    writer.writerow([
                        ...
                        str(float(max(max_reg_vals))), 'True', str(time.time() - start),
                    ...
                    writer.writerow([
                        ...
                        str(float(exp_rew)),
                        str(float(max(abs(bf_val)))),
                        str(float(max(max_reg_vals))), 'False',str(time.time() - start),
                        str(float(max(max_reg_vals_update))),
                    ])
                    f.close()
                    #break
        ...
        #         writer2.writerow([])
        np.save(policy_file_name+'--'+str(epc+1)+'.npy',om)


if __name__ == "__main__":
    for _ in range(20):
        main('MaxRew','constrained','FairGamble','MDCE')
        main('MaxRew','constrained','Hunt','value-10',5)
        main('MaxRew','constrained','CaE','balance',25)
